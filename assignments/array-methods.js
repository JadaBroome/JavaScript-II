...
let fullName = [];
const mapFullName = runners.map((arr) => {
    return {'First Name': arr.first_name, 'Last Name': arr.last_name};
});

fullName.push(mapFullName);
console.log(fullName);

// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runner's first names converted to uppercase because the director BECAME DRUNK WITH POWER. Convert each first name into all caps and log the result
let allCaps = [];
const mapAllCaps = runners.map((arr) => {
    return{'First Name': arr.first_name.toUpperCase()}
});
allCaps.push(mapAllCaps);
console.log(allCaps); 

// ==== Challenge 3: Use .filter() ====
// The large shirts won't be available for the event due to an ordering issue.  Get a list of runners with large sized shirts so they can choose a different size. Return an array named largeShirts that contains information about the runners that have a shirt size of L and log the result
let largeShirts = [];
const filterLargeShirts = runners.filter((arr) => {
...
const reduceTicketPriceTotal = runners.reduce((total, arr) => {
    return total += arr.donation;
}, 0);
ticketPriceTotal.push(reduceTicketPriceTotal);
...
// Problem 1
let companyName =[];
const filterCompanyName = runners.filter((arr) => {
    return arr.company_name === 'Skinix' || arr.company_name ==='Skinte';
})
companyName.push(filterCompanyName);
console.log(companyName);

// Problem 2
let lastNames= [];
const sortLastNames = runners.map((arr) => {
    return arr.last_name 
});
sortLastNames.sort();
lastNames.push(sortLastNames);
